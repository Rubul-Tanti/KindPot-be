    ...
    if (existingUser) {
      ...
      logger.info(`google login success for ${existingUser.email}`);
            ...
            if(existingUser.role!=='ADMIN'){
               if(!ActiveSubs){
                    const vuser=await prisma.user.update({where:{id:existingUser.id},data:{role:"VIEWER"}})
                }else{
                    const vuser=await prisma.user.update({where:{id:existingUser.id},data:{role:"SUBSCRIBER"}})
                }
            }
